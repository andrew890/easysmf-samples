...
/**
 * CICS transaction statistics report grouped by APPLID and TRAN.
 */
public class CicsTransactionStats 
{
    public static void main(String[] args) throws IOException 
    {
        if (args.length < 2)
        {
            System.out.println("Usage: CicsTransactionStats <dictionary-name> <input-name> <input-name2> ...");
        ...
        Map<ApplidTran, TransactionStatsData> data = new HashMap<>();
                ...
                for (SmfRecord smfrecord : reader) 
                {
                    ...
                    if (r110.haveDictionary()) 
                    {
                        ...
                        for (PerformanceRecord txData : r110.performanceRecords()) 
                        {
                            ...
                            data.computeIfAbsent(key, k -> new TransactionStatsData()).add(txData);
                        }
                    } 
                    else 
                    {
    ...
    private static void writeReport(Map<ApplidTran, TransactionStatsData> data) 
    {
        data.entrySet().stream()
            ...
            .forEachOrdered(applidSection -> 
            {
                ...
                applidSection.getValue().stream()
                    ...
                    .forEachOrdered(e -> 
                    {
                        TransactionStatsData tx = e.getValue();
    ...
    private static class TransactionStatsData 
    {
